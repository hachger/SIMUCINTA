...
uint8_t UNERBUS_SendToBuf(_sUNERBUSHandle *aBus, uint8_t cmdID, uint8_t lenCMD, uint8_t *bufForSend){
	uint8_t i;
...
uint8_t UNERBUS_IsNewData(_sUNERBUSHandle *aBus){
    return (aBus->rx.newData);
}
